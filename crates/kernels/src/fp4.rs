...
mod tests {
    ...
    /// the bug investigation, not the round-number 0.5/2.0 scales the other
    /// tests in this file use), computes the GEMM's raw (pre-alpha)
    /// block-scaled product entirely on the host -- exactly mirroring what
    ...
    fn gemm_alpha_must_divide_by_input_scale_not_just_multiply_weight_scale_2() {
        ...
        // all-equal), both signs, RMS on the order of 1-2 -- roughly the
        // magnitude of a real post-`rms_norm` hidden state.
        let x: Vec<f32> = (0..k)
            .map(|i| {
                let t = i as f32;
        ...
        // DISTINCT per-block scale bytes (so a block-indexing bug would show
        // up too), and lm_head's own real, measured `weight_scale_2`.
        let weight_scale_2 = 0.000_127_156_57_f32;
        // Block 0: codes alternating 1.0 (0b0010) / -1.5 (0b1011), scale=3.0.
        // Block 1: codes alternating 2.0 (0b0100) / -0.5 (0b1001), scale=1.25.
        let byte_b0 = (0b1011u8 << 4) | 0b0010u8;
        let byte_b1 = (0b1001u8 << 4) | 0b0100u8;
        let packed_w: Vec<u8> = std::iter::repeat_n(byte_b0, 8)
            .chain(std::iter::repeat_n(byte_b1, 8))
            .collect();
        let scale_w_f32 = vec![3.0f32, 1.25f32];

        let raw_b = dequant_f4e2m1_row(&packed_w, &scale_w_f32, 1.0, k);
        let w_true = dequant_f4e2m1_row(&packed_w, &scale_w_f32, weight_scale_2, k);

        let gemm_raw: f64 = raw_a
            .iter()
            .zip(raw_b.iter())
        ...
        let true_dot: f64 = x
            ...
            .sum();
        // A dot product with real, healthy dimensionality has partial
        // cancellation between positive and negative terms (exactly what a
        // real lm_head logit is), which makes error-RELATIVE-TO-THE-NET-SUM
        // an unstable, misleading metric if the sum happens to land near
        // zero -- this is a property of the near-cancelling SUM, not of the
        // per-element quantization error. Scale tolerances against the
        // dot product's own OPERAND magnitude instead (`sum(|x_i * w_i|)`),
        // a stable quantity that does not depend on how much cancellation
        // the particular chosen values happen to produce.
        let magnitude: f64 = x
            .iter()
            .zip(w_true.iter())
