...
/// `ATTN_DSPLIT_HALF_NTILES` supports; see `attn_prefill_decoupled_f32`'s
/// doc comment in `ops.cu`). Single-chunk only, no `partial` reduction --
/// that's not built yet.
#[test]
fn attn_prefill_decoupled_matches_the_reference() -> Result<()> {
    if std::env::var("INFERO_ATTN_MMA").as_deref() != Ok("1") {
        eprintln!("skipping: needs INFERO_ATTN_MMA=1 set before the test binary starts");
        return Ok(());
    }
    ...
    // tpw = (16/group).max(1) = (16/6).max(1) = 2 for this shape, so a tile
    // is 2 tokens x 6 heads = 12 rows. Cover under, at, and past one tile,
    // plus a longer run with a real multi-tile kv_len.
    for (run_tokens, kv_len) in [(1usize, 40usize), (2, 33), (3, 33), (16, 100), (63, 4000)] {
        ...
        assert!(
            abs < 2e-3,
            "attn_prefill_decoupled run {run_tokens} kv {kv_len}: \
             max abs diff {abs} at {at} (got {}, want {})",
            got[run_lo + at],
            want[run_lo + at]
        );
        ...
        assert_eq!(
            &got[..run_lo],
            &want[..run_lo],
            "attn_prefill_decoupled run {run_tokens} kv {kv_len}: wrote before the run"
        );
        assert_eq!(
            &got[run_hi..],
            &want[run_hi..],
            "attn_prefill_decoupled run {run_tokens} kv {kv_len}: wrote past the run"
        );
    }
    Ok(())
}

/// `attn_prefill_decoupled2` -- the T=2 generalization (2 tile-groups
/// sharing one K/V stream per block) -- against the same reference.
/// Shapes cover under/at/past one AND two tiles (a `big_tile` covers
/// `2*tpw` tokens = 4 for this shape), plus a longer multi-`n_blk` run.
#[test]
fn attn_prefill_decoupled2_matches_the_reference() -> Result<()> {
    if std::env::var("INFERO_ATTN_MMA").as_deref() != Ok("1") {
        eprintln!("skipping: needs INFERO_ATTN_MMA=1 set before the test binary starts");
        return Ok(());
    }
    ...
    for (run_tokens, kv_len) in [(1usize, 40usize), (2, 33), (3, 33), (4, 33), (5, 100), (16, 100), (63, 4000)] {
        ...
        assert!(
            abs < 2e-3,
            "attn_prefill_decoupled2 run {run_tokens} kv {kv_len}: \
             max abs diff {abs} at {at} (got {}, want {})",
            got[run_lo + at],
            want[run_lo + at]
        );
        ...
        assert_eq!(
            &got[..run_lo],
            &want[..run_lo],
            "attn_prefill_decoupled2 run {run_tokens} kv {kv_len}: wrote before the run"
        );
        assert_eq!(
            &got[run_hi..],
            &want[run_hi..],
            "attn_prefill_decoupled2 run {run_tokens} kv {kv_len}: wrote past the run"
        );
    }
    Ok(())
}

/// `attn_prefill_decoupled2_f16acc` -- T=2 with the PV-consumer's `o[]`
/// accumulated in fp16 instead of fp32. Same shapes as T=2's own test, but
/// a looser tolerance: fp16 accumulation over many K-tile iterations (the
/// `kv_len=4000` case walks `n_blk` up into the hundreds) compounds real,
/// if small, per-step rounding in a way fp32 accumulation doesn't -- this
/// is the real accuracy cost of the register saving, not a bug, and needs
/// its own honest tolerance rather than reusing fp32's.
#[test]
fn attn_prefill_decoupled2_f16acc_matches_the_reference() -> Result<()> {
    if std::env::var("INFERO_ATTN_MMA").as_deref() != Ok("1") {
        eprintln!("skipping: needs INFERO_ATTN_MMA=1 set before the test binary starts");
        return Ok(());
    }
    ...
    for (run_tokens, kv_len) in [(1usize, 40usize), (2, 33), (3, 33), (4, 33), (5, 100), (16, 100), (63, 4000)] {
        ...
        assert!(
            abs < 1e-2,
            "attn_prefill_decoupled2_f16acc run {run_tokens} kv {kv_len}: \
             max abs diff {abs} at {at} (got {}, want {})",
            got[run_lo + at],
            want[run_lo + at]
        );
        ...
        assert_eq!(
            &got[..run_lo],
            &want[..run_lo],
            "attn_prefill_decoupled2_f16acc run {run_tokens} kv {kv_len}: wrote before the run"
        );
        assert_eq!(
            &got[run_hi..],
            &want[run_hi..],
