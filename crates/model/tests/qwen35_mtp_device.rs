...
fn the_drafters_cache_sits_one_position_behind_the_targets() -> Result<()> {
    ...
    with_capture("drafter cache positions", |c| {
        ...
        assert_eq!(head.cache_len(0), 0, "a fresh drafter has no history");
        let positions: Vec<usize> = (0..t).collect();
        head.step(&kern, &embed, &ids, &positions, &hidden.as_view(), None, 0)?;
        ...
        // sampled.
        assert_eq!(head.cache_len(0), t, "the drafter's cache should hold {t}");
        ...
        head.step_from_own_output(&kern, &embed, ids[0], t, t - 1, 0)?;
        assert_eq!(head.cache_len(0), t + 1);
        assert_eq!(head.rows(), 1);

        // And rolling back is expressible: the drafter's own coordinate system.
        head.truncate(0, t);
        assert_eq!(head.cache_len(0), t);
        Ok(())
    })
}
...
fn the_head_reaches_a_useful_acceptance_length_at_k_2() -> Result<()> {
    ...
    with_capture("k = 2 acceptance", |c| {
        ...
        for row in 0..t {
            ...
            head.step_from_own_output(&kern, &embed, first[row], row + 1, row, 0)?;
...
fn chunked_priming_agrees_with_one_wide_step() -> Result<()> {
    ...
    with_capture("chunked priming", |c| {
        ...
        let (embed, ids) = stub_embedding(&dev, c)?;
        let last = head.prime(&kern, &embed, &ids, &positions, &hidden.as_view(), None, 0)?;
        ...
        assert_eq!(head.cached(0), t, "the drafter's cache after priming");
