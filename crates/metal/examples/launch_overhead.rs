...
fn main() -> Result<()> {
    ...
    // Isolate the allocation-per-scalar-arg cost `Arg::Bytes(Vec<u8>)` pays:
    // this loop does the same N pushes but discards the queue's realloc-heavy
    // path by using a kernel with more scalar args than `add`'s one, so the
    // per-launch scalar allocation count roughly matches a Q4_K gemv call
    // (weight, activation, k, n, n_tokens -- three scalars).
    stream.synchronize()?;
    let t = std::time::Instant::now();
    for _ in 0..N {
    ...
    stream.synchronize()?;
    println!(
        "\n{N} rms_norm launches (2 scalar args instead of add's 1)"
    );
    println!(
        "  cpu submit : {:>8.2} ms   ({:.2} us / launch)",
