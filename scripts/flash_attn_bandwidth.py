...
Companion to `marlin_bandwidth.py`, and written for the same reason: tuili's
three attention kernels have only ever been measured against each other and
against tuili's own fused attempt, which cannot say how much of the gap to vLLM
...
The tuili number this was first written against — 85.8 us a layer against
vLLM's 58.1 — was taken under `TUILI_PROFILE`, which serializes the stream and
puts a CUDA event pair around every launch. Measured the way this script
measures, by the host clock over back-to-back launches, tuili's three kernels
...
def main():
    ...
    print("  tuili, same shape: cargo test -p tuili-kernels --test bwidth_attn "
          f"-- --nocapture, with TUILI_ATTN_HIST={HISTORY}")
