...
**Amended again — that follow-up has now landed** (`tq_long_run_kernel` in `crates/model/src/lib.rs`, plus a persistent `Model::hw_caps`). The long path now makes the same three-way choice the F16 dense path does: flash_attn2, then `decoupled6` (`d_head == 256`, `INFERO_PREFILL_T6=0` to roll back), then `ws4`. The one thing it deliberately does **not** copy from the F16 path is that path's `attn_backend_name != "handrolled"` gate. `attn_backend_name` is resolved once at load against the *pool's* real `KvQuant`, and `FlashAttn2Ffi::supports()` declines any quantized one — so on a TurboQuant model it is always `"handrolled"`, and gating on it would answer a question about the pool's storage format when the real question is about the *dequantized f16 scratch buffer* this call is about to hand the kernel, which is not quantized at all. The long path therefore calls `supports()` itself, per run, passing `KvCacheQuant::F16` explicitly — the same function, called with the parameter that is true for this call's real data, not a fork of its logic. Nothing changes for `d_head != 256` (the local test checkpoint, and every shape this plan's own GPU tests exercise): both new branches are hard-gated at 256, so `ws4` still serves them all. See `tq-fa2-followup-report.md` for what that follow-up could and could not verify.
